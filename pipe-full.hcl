...
## I first begin with the adding IIADDQ to the instr_valid. My iaddq function takes 1 byte icode:ifun,1 byte F:rB;and 8 byte immediate value which going to be ValC in fetch cycle.
## Since I add it also to the need_regids and need_valC.In decode stage since we need only valB, we can add IADDQ to src B, also in the write-back stage since we will write valE to rb, I add IADDQ to dest_e.
## In the execute stage we add ValB to ValC and put it into ValE.So, as a alu_A we use E_valC and as a aluB we use E_valB.Finally we set conditions in the execute stage by putting IADDQ into set_cc.
####################################################################
